...
//    List<Datum> datumList;
    ...
    private ArrayList datumList = new ArrayList<String>();
...
//        mockResponse.setBody(jsonString);
        ...
        if (jsonArray != null){
            ...
            for (int i=0;i < len;i++){
                ...
                datumList.add(element.getAsString());
            }
        }
...
//        datumList = employeeResponse.getData();
        ...
        assertEquals("Tiger Nixon",datumList.get(0));
        assertEquals("320800",datumList.get(0));

        Iterator<Datum> datumIterator = Mockito.mock(Iterator.class);
        for (int i=0;i<datumList.size();i++) {
            Mockito.when(datumIterator.hasNext()).thenReturn(true,false);
            assertEquals(getDummy_EmployeeList().get(i),datumList.get(i));
        }
    }
